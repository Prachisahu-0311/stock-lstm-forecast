...
def main():
    st.title("📈 Stock Price Forecasting Dashboard – ARIMA vs Prophet vs LSTM")
    st.write(
    ...
    if st.sidebar.button("Run Forecast"):
       ...
       # LSTM IMPLEMENTATION
        if run_lstm:
            st.subheader("🔹 LSTM Model with Technical Indicators")
        ...
        if model_predictions:
            st.subheader("📉 Comparison: Actual vs Model Forecasts (Test Window)")
            fig, ax = plt.subplots(figsize=(12, 5))
            ax.plot(test_index, test_close.values[-test_size:], label="Actual Close", color="black")

            for name, preds in model_predictions.items():
                # Align lengths – LSTM may be based on slightly shifted index, use last test_size indices
                ax.plot(test_index, preds[-test_size:], label=name)

            ax.set_xlabel("Date")
            ax.set_ylabel("Price")
            ax.set_title(f"{ticker} – Model Forecast Comparison (Last {test_size} days)")
            ax.legend()
            st.pyplot(fig)
